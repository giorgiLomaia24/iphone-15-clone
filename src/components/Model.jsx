        ...
        <div className="flex flex-col items-center mt-5">
          <div className="w-full h-[75vh] md:h-[90vh] overflow-hidden relative">
            ...
            />
              ...
              setrotations={setLargeRotation}
              item={model}
              size={size}
            />
            <Canvas className='w-full h-full'
              style={{ position: 'fixed', left: 0, top: 0, right: 0, bottom: 0, overflow: 'hidden' }}
            ...
            <div className="flex-center">
              ...
              <button className="size-btn-container">
                {sizes.map(({label, value}) => (
                  <span key={value} className="cursor-pointer size-btn" style={{backgroundColor: size === value ? 'white' : 'transparent', color: size === value ? 'black' :'white'}} onClick={() => setSize(value)}>{label }</span>
                ))}
              </button>
            </div>
          </div>
