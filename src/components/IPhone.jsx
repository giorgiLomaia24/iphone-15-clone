...
function Model(props) {
    ...
    useEffect(() => {
        if (
          props.item &&               // Check if props.item is defined
          Array.isArray(props.item.color) &&  // Check if props.item.color is an array
          props.item.color.length > 0  // Check if the array has at least one element
        ) {
          Object.entries(materials).map((material) => {
            if (
              ...
              material[1].color = new THREE.Color(props.item.color[0]);  // Assign the first color
            }
            material[1].needsUpdate = true;  // Mark material for update
          });
        }
